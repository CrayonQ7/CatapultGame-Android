...
bool GameLayer::init(int pl1, int pr1, int pl2, int pr2, int bg, float bv, float ev, int bgm)
{
	...
	//tmx->setScale(visibleSize.height / tileHeight);
	...
	auto joystick = Sprite::create("JoyStickJ.png");
	auto joystickBg = Sprite::create("JoystickBg.png");
	joystick->setScale(2);
	joystickBg->setScale(2);
	joy = Joystick::create(joystickBg->getContentSize().width, joystick, joystickBg);
	this->addChild(joy);


	auto pauseItem = MenuItemSprite::create(
		Sprite::create("icons/PauseNormal.png"),
		Sprite::create("icons/PauseSelected.png"),
		CC_CALLBACK_1(GameLayer::pauseCallBack, this));
	pauseItem->setPosition(Vec2(origin.x + visibleSize.width / 2, origin.y + visibleSize.height - pauseItem->getContentSize().height / 2));
	...
	sprintf(file1, "prop_%d_1.png", pr1);
	sprintf(file2, "prop_%d_2.png", pr1);
	auto propLeftItem = MenuItemSprite::create(
		Sprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(file1)),
		Sprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(file2)),
		CC_CALLBACK_1(GameLayer::propLeft, this));
	propLeftItem->setPosition(origin.x + propLeftItem->getContentSize().width / 2, origin.y + propLeftItem->getContentSize().height/2);

	sprintf(file1, "prop_%d_1.png", pr2);
	sprintf(file2, "prop_%d_2.png", pr2);
	auto propRightItem = MenuItemSprite::create(
		Sprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(file1)),
		Sprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(file2)),
		CC_CALLBACK_1(GameLayer::propRight, this));
	propRightItem->setPosition(origin.x + visibleSize.width - propRightItem->getContentSize().width / 2, origin.y + propRightItem->getContentSize().height/2);
	auto menuProp = Menu::create(propLeftItem, propRightItem, NULL);
	menuProp->setPosition(Vec2::ZERO);
	addChild(menuProp);

	this->scheduleUpdate();
	this->scheduleOnce(schedule_selector(GameLayer::updateOnce), 0.5f);
	return true;
}
...
void GameLayer::propLeft(Ref * pSender)
{
	if (hero1->mpBar->getPercentage() > 0 && hero1->canUseProp)
		hero1->pUseProp();
}

void GameLayer::propRight(Ref * pSender)
{
