...
bool OperateLayer::init(int bgm, int mode)
{
	...
	do {
		...
		auto touchListener2 = EventListenerTouchOneByOne::create() ;  // 创建多点触摸监听器
		...
		//auto touchesListener = EventListenerTouchAllAtOnce::create();  // 鍒涘缓澶氱偣瑙︽懜鐩戝惉鍣?
...
bool OperateLayer::onTouchBegan(cocos2d::Touch * touch, cocos2d::Event * event)
{
	...
	if (mode == 1)
	{
		schedule(schedule_selector(OperateLayer::updatePercent1), 0.1f); // 开始蓄力
